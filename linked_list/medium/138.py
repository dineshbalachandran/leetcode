...
each new node has its value set to the value of its corresponding original node. Both the next and random pointer of the new nodes should point to new nodes in the copied list such that the pointers in the original list and copied list represent the same list state. None of the pointers in the new list should point to nodes in the original list.
...
class Solution:
    def copyRandomList(self, head):
        ...
        while src:            
            n = nodemap[src] if src in nodemap else Node(src.val)                        
            nodemap[src] = n
            
            n.random = nodemap[src.random] if src.random in nodemap else Node(src.random.val)
            nodemap[src.random] = n.random
